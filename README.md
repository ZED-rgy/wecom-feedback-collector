...
发送任务会保存目标群快照。更换监听群时，旧群的待发送任务会自动取消；历史版本中没有目标群快照的任务也会被安全取消，避免切群后误发。相同群、相同摘要在短时间内重复点击只会复用同一个活动任务。普通发送异常最多自动重试 3 次，之后转为“发送失败”；“待人工确认”任务不会自动重试。
...
配置保存会校验群名、时间、轮询间隔、摘要模板变量和文本长度；写入 `.env` 使用临时文件替换，避免进程中断留下半份配置。桌面日志使用轮转文件，默认保留 5 个历史文件。
